...
def run_dha_tool(
    ...
    Args:
        ...
        plan_reinforcement: Generate and verify a grid reinforcement plan
        verbose: Enable verbose logging
    ...
    if plan_reinforcement:
        cmd.append("--plan-reinforcement")
    ...
    try:
        ...
        outputs = {
            ...
            "reinforcement": (output_dir / "dha_reinforcement.json").exists(),
            "reinforced_kpis": (output_dir / "dha_reinforced_kpis.json").exists(),
        }
...
def run_economics_tool(
    ...
    try:
        ...
        if scenario:
            ...
            output_dir = output_dir / "scenarios" / scenario_name(scenario)
        outputs = {
...
def run_decision_tool(
    ...
    if llm_explanation:
        ...
        # Keep the physics/decision run deterministic and do not accidentally
        # call Gemini merely because the CLI's general default format is
        # ``all``.  Existing explanation sidecars must not be reported as if
        # they were generated by this JSON-only invocation.
    ...
    if scenario:
        from branitz_heat_decision.economics.scenarios import scenario_name
        cmd.extend(["--scenario", scenario_name(scenario)])
    ...
    try:
        ...
        outputs = {
            ...
            "explanation": bool(llm_explanation) and (
                output_dir / f"explanation_{cluster_id}.md"
            ).exists(),
            "validation": bool(llm_explanation) and (
                output_dir / f"validation_{cluster_id}.json"
            ).exists(),
        }
